...
function* getCategories({ payload }) {
	try {
		...
		const uri = `${ENV.CURRENT_API}${ENDPOINTS.CATEGORIES}`;
		...
		yield put({
			type: ACTION_TYPES.CATEGORIES.GET_LOAD,
		});
		...
		if (timeout) {
			yield put({
				type: ACTION_TYPES.CATEGORIES.GET_ERR,
			...
			yield put({
				type: ACTION_TYPES.CATEGORIES.GET_RES,
				payload: result,
			});
		}
	} catch (err) {
...
export default function* combineSaga() {
	yield takeEvery(ACTION_TYPES.CATEGORIES.GET, getCategories);
}
